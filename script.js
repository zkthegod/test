document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateEffects() {
        ...
        const colors = [];
        // Scope to Name Effects colors container only
        const scopedPickers = gradColorsContainer ? gradColorsContainer.querySelectorAll('.color-picker') : [];
        scopedPickers && scopedPickers.forEach(picker => { if (picker.value) colors.push(picker.value); });
        const angle = parseFloat(gradDirection?.value || '90');
        const glow = glowColor?.value || '#000000';
        const speed = waveSpeed?.value || '';

        if (colors.length > 1) {
            // Build repeating gradient with multiple colors
            const gradientStops = [];
            const step = 100 / colors.length;
            for (let i = 0; i < colors.length; i++) {
                gradientStops.push(`${colors[i]} ${i*step}%`);
            }
            ...
            // ensure text uses gradient fill
            effectPreview.style.setProperty('-webkit-text-fill-color', 'transparent');
        } else if (colors.length === 1) {
            effectPreview.style.backgroundImage = `linear-gradient(90deg, ${colors[0]}, ${colors[0]})`;
            effectPreview.style.backgroundRepeat = 'no-repeat';
            effectPreview.style.animation = 'none';
            effectPreview.style.setProperty('-webkit-text-fill-color', 'transparent');
        } else {
            // Fallback: visible solid text color if no colors available
            effectPreview.style.backgroundImage = 'none';
            effectPreview.style.removeProperty('background-repeat');
            effectPreview.style.removeProperty('background-size');
            effectPreview.style.removeProperty('animation');
            effectPreview.classList.remove('wave-normal', 'wave-slow', 'wave-very-slow','wave-fast','wave-very-fast');
            effectPreview.style.color = getComputedStyle(document.documentElement).getPropertyValue('--text') || '#fff';
            effectPreview.style.setProperty('-webkit-text-fill-color', 'currentColor');
        }
    ...
    if (avatarData.length === 0) {
        // Full static list fallback (subset shown; extend as needed)
        avatarData = [
            { name: "Homer Simpson", file: "Simpsons-Homer-icon.png", category: "simpsons" },
            { name: "Marge Simpson", file: "Simpsons-Marge-icon.png", category: "simpsons" },
            { name: "Bart Simpson", file: "Simpsons-Bart-icon.png", category: "simpsons" },
            { name: "Lisa Simpson", file: "Simpsons-Lisa-icon.png", category: "simpsons" },
            { name: "Maggie Simpson", file: "Simpsons-Maggie-icon.png", category: "simpsons" },
            { name: "Pikachu", file: "Pokemon-Pikachu-icon.png", category: "pokemon" },
            { name: "Ash Ketchum", file: "Pokemon-Ash-icon.png", category: "pokemon" },
            { name: "Smurfette", file: "Smurfs-Smurfette-icon.png", category: "smurf" },
            { name: "Dilbert", file: "Dilbert-Dilbert-icon.png", category: "dilbert" },
            { name: "South Park Stan", file: "SouthPark-Stan-icon.png", category: "southpark" },
            { name: "Garfield", file: "Garfield-Garfield-icon.png", category: "garfield" },
            { name: "Grimm", file: "Grimm-Grimm-icon.png", category: "grimm" },
            { name: "Sesame Elmo", file: "Sesame-Elmo-icon.png", category: "sesame" },
            { name: "Giant Robo", file: "GiantRobo-Icon.png", category: "giantrobo" },
            { name: "Sherman", file: "Shermans-Sherman-icon.png", category: "shermans" },
        ];
    }
